...
int MIDPOINT::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  PlotLine *line = getMIDPOINT(in, period);
  if (! line)
  {
    delete in;
    return 1;
...
int MIDPOINT::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
...
PlotLine * MIDPOINT::getMIDPOINT (PlotLine *in, int period)
{
  int size = in->getSize();
  TA_Real input[size];
  TA_Real out[size];
  ...
  TA_Integer outBeg;
  TA_Integer outNb;
  TA_RetCode rc = TA_MIDPOINT(0, size - 1, &input[0], period, &outBeg, &outNb, &out[0]);
  if (rc != TA_SUCCESS)
  {
