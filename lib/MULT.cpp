...
int MULT::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,MULT,<NAME>,<INPUT_1>,<INPUT_2>

  if (set.count() != 5)
  {
    qDebug() << "MULT::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  TA_RetCode rc = TA_MULT(0, size - 1, &input[0], &input2[0], &outBeg, &outNb, &out[0]);

  if (rc != TA_SUCCESS)
  {
    qDebug() << "MULT::calculate: TA-Lib error" << rc;
    return 1;
  }
