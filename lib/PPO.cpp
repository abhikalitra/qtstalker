...
int PPO::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  PlotLine *line = getPPO(in, fast, slow, ma);
  if (! line)
  {
    delete in;
    return 1;
...
int PPO::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
...
PlotLine * PPO::getPPO (PlotLine *in, int fast, int slow, int ma)
{
  TA_Integer outBeg;
  TA_Integer outNb;
