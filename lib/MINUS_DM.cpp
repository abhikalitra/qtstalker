...
int MINUS_DM::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,MINUS_DM,<NAME>,<PERIOD>

  if (set.count() != 4)
  {
    qDebug() << "MINUS_DM::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "MINUS_DM::calculate: invalid period parm" << set[3];
    return 1;
  }
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << "MINUS_DM::calculate: TA-Lib error" << rc;
    return 1;
  }
