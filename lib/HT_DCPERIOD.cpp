...
int HT_DCPERIOD::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,HT_DCPERIOD,<NAME>,<INPUT>

  if (set.count() != 4)
  {
    qDebug() << "HT_DCPERIOD::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << "HT_DCPERIOD::calculate: TA-Lib error" << rc;
    return 1;
  }
