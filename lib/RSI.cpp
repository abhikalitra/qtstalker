...
int RSI::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,RSI,<NAME>,<INPUT>,<PERIOD>

  if (set.count() != 5)
  {
    qDebug() << "RSI::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "RSI::calculate: invalid period parm" << set[4];
    return 1;
  }
  ...
  TA_RetCode rc = TA_RSI(0, in->getSize() - 1, &input[0], period, &outBeg, &outNb, &out[0]);
  if (rc != TA_SUCCESS)
  {
