...
CommandPlotHistogramBar::CommandPlotHistogramBar (QObject *p) : Command (p)
{
  _type = "PLOT_HISTOGRAM_BAR";
}

int CommandPlotHistogramBar::runScript (void *d)
{
  ...
  Curve *in = script->curve(name);
  if (! in)
  {
    qDebug() << _type << "::run: INPUT not found" << name;
    return _ERROR;
  }
  ...
  Curve *line = new CurveHistogram("Histogram Bar");
...
SettingGroup * CommandPlotHistogramBar::settings ()
{
