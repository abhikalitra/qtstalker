...
int MFI::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,MFI,<NAME>,<PERIOD>

  if (set.count() != 4)
  {
    qDebug() << "MFI::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "MFI::calculate: invalid period parm" << set[3];
    return 1;
  }
