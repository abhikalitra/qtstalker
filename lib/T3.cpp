...
int T3::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,T3,<NAME>,<INPUT>,<PERIOD>,<VFACTOR>

  if (set.count() != 6)
  {
    qDebug() << "T3::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "T3::calculate: invalid period parm" << set[4];
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "T3::calculate: invalid period parm" << set[5];
    return 1;
  }
