...
QString CommandATR::run (CommandParse &, void *d)
{
  ...
  {
    qDebug() << "CommandATR::run: invalid HIGH" << td.toString();
    return _returnCode;
  }
  ...
  {
    qDebug() << "CommandATR::run: invalid LOW" << td.toString();
    return _returnCode;
  }
  ...
  {
    qDebug() << "CommandATR::run: invalid CLOSE" << td.toString();
    return _returnCode;
  }
  ...
  parms.set(QString("FUNCTION"), Data(TypeTALibFunction::_ATR));
  
  CurveData line;
  TALibFunction func;
  if (func.run(parms, 3, ihigh, ilow, iclose, iclose, 1, line, line, line))
  {
    qDebug() << "CommandATR::runScript: getATR error";
    return _returnCode;
  }
