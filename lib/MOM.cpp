...
int MOM::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,MOM,<NAME>,<INPUT>,<PERIOD>

  if (set.count() != 5)
  {
    qDebug() << "MOM::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "MOM::calculate: invalid period parm" << set[4];
    return 1;
  }
  ...
  TA_RetCode rc = TA_MOM(0, in->getSize() - 1, &input[0], period, &outBeg, &outNb, &out[0]);
  if (rc != TA_SUCCESS)
  {
