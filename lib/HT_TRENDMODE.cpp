...
int HT_TRENDMODE::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  PlotLine *line = getHT_TRENDMODE(in);
  if (! line)
  {
    delete in;
    return 1;
...
int HT_TRENDMODE::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,HT_TRENDMODE,<NAME>,<INPUT>
...
PlotLine * HT_TRENDMODE::getHT_TRENDMODE (PlotLine *in)
{
  TA_Integer outBeg;
  TA_Integer outNb;
  ...
  TA_RetCode rc = TA_HT_TRENDMODE(0, in->getSize() - 1, &input[0], &outBeg, &outNb, &out[0]);
  if (rc != TA_SUCCESS)
  {
