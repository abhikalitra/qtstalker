...
int LINEARREG_SLOPE::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  PlotLine *line = getLINEARREG_SLOPE(in, period);
  if (! line)
  {
    delete in;
    return 1;
...
int LINEARREG_SLOPE::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,LINEARREG_SLOPE,<NAME>,<INPUT>,<PERIOD>
...
PlotLine * LINEARREG_SLOPE::getLINEARREG_SLOPE (PlotLine *in, int period)
{
  int size = in->getSize();
  TA_Real input[size];
  TA_Real out[size];
  ...
  TA_Integer outBeg;
  TA_Integer outNb;
  TA_RetCode rc = TA_LINEARREG_SLOPE(0, size - 1, &input[0], period, &outBeg, &outNb, &out[0]);
  if (rc != TA_SUCCESS)
  {
