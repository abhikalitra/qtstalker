...
int PLUS_DI::getIndicator (Indicator &ind, BarData *data)
{
  int period = settings.getInt(periodKey);

  PlotLine *line = getPLUS_DI(data, period);
  if (! line)
    return 1;
...
int PLUS_DI::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
...
PlotLine * PLUS_DI::getPLUS_DI (BarData *data, int period)
{
  int size = data->count();
  TA_Real high[size];
  TA_Real low[size];
