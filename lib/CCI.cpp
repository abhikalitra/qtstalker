...
int CCI::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,CCI,<NAME>,<PERIOD>

  if (set.count() != 4)
  {
    qDebug() << "CCI::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "CCI::calculate: invalid period parm" << set[3];
    return 1;
  }
