...
int MACDFIX::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  int period = settings.getInt(sigpdKey);

  QList<PlotLine *> l;
  int rc = getMACDFIX(in, period, l);
  if (rc)
  {
    qDeleteAll(l);
    delete in;
...
int MACDFIX::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
...
int MACDFIX::getMACDFIX (PlotLine *in, int signal, QList<PlotLine *> &l)
{
  int size = in->getSize();
  TA_Integer outBeg;
  TA_Integer outNb;
  ...
  TA_RetCode rc = TA_MACDFIX(0, size - 1, &input[0], signal, &outBeg, &outNb, &out[0], &out2[0], &out3[0]);
  if (rc != TA_SUCCESS)
  {
