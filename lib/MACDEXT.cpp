...
int MACDEXT::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  QList<PlotLine *> l;
  int rc = getMACDEXT(in, fast, fastma, slow, slowma, signal, signalma, l);
  if (rc)
  {
    qDeleteAll(l);
    delete in;
...
int MACDEXT::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
...
int MACDEXT::getMACDEXT (PlotLine *in, int fast, int fma, int slow, int sma, int signal,
