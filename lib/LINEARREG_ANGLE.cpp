...
int LINEARREG_ANGLE::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  PlotLine *line = getLINEARREG_ANGLE(in, period);
  if (! line)
  {
    delete in;
    return 1;
...
int LINEARREG_ANGLE::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,LINEARREG_ANGLE,<NAME>,<INPUT>,<PERIOD>
...
PlotLine * LINEARREG_ANGLE::getLINEARREG_ANGLE (PlotLine *in, int period)
{
  int size = in->getSize();
  TA_Real input[size];
  TA_Real out[size];
  ...
  TA_Integer outBeg;
  TA_Integer outNb;
  TA_RetCode rc = TA_LINEARREG_ANGLE(0, size - 1, &input[0], period, &outBeg, &outNb, &out[0]);
  if (rc != TA_SUCCESS)
  {
