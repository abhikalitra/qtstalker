...
int WILLR::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,WILLR,<NAME>,<PERIOD>

  if (set.count() != 4)
  {
    qDebug() << "WILLR::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "WILLR::calculate: invalid period parm" << set[3];
    return 1;
  }
