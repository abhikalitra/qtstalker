...
int ADOSC::getIndicator (Indicator &ind, BarData *data)
{
...
int ADOSC::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  ...
  if (! ok)
  {
    qDebug() << indicator << "::calculate: invalid fast period settings" << set[3];
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << indicator << "::calculate: invalid slow period settings" << set[4];
    return 1;
  }
...
PlotLine * ADOSC::getADOSC (BarData *data, int fast, int slow)
{
  int size = data->count();
  TA_Real high[size];
  TA_Real low[size];
