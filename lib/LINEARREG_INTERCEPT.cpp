...
int LINEARREG_INTERCEPT::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  PlotLine *line = getLINEARREG_INTERCEPT(in, period);
  if (! line)
  {
    delete in;
    return 1;
...
int LINEARREG_INTERCEPT::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,LINEARREG_INTERCEPT,<NAME>,<INPUT>,<PERIOD>
...
PlotLine * LINEARREG_INTERCEPT::getLINEARREG_INTERCEPT (PlotLine *in, int period)
{
  int size = in->getSize();
  TA_Real input[size];
  TA_Real out[size];
  ...
  TA_Integer outBeg;
  TA_Integer outNb;
  TA_RetCode rc = TA_LINEARREG_INTERCEPT(0, size - 1, &input[0], period, &outBeg, &outNb, &out[0]);
  if (rc != TA_SUCCESS)
  {
...
int LINEARREG_INTERCEPT::dialog ()
{
  int page = 0;
  QString k, d;
