...
int SUM::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  PlotLine *line = getSUM(in, period);
  if (! line)
  {
    delete in;
    return 1;
...
int SUM::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
...
PlotLine * SUM::getSUM (PlotLine *in, int period)
{
  int size = in->getSize();
  TA_Real input[size];
  TA_Real out[size];
  ...
  TA_Integer outBeg;
  TA_Integer outNb;
  TA_RetCode rc = TA_SUM(0, size - 1, &input[0], period, &outBeg, &outNb, &out[0]);
  if (rc != TA_SUCCESS)
  {
