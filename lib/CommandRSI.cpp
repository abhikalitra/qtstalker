...
void CommandRSI::runScript (CommandParse sg, Script *script)
{
  if (Command::parse(sg, script))
  {
    Command::error("CommandRSI::runScript: parse error");
    return;
  }
  ...
  if (! in)
  {
    Command::error("CommandRSI::runScript: invalid Input");
    return;
  }
  ...
  Data *line = getRSI(list, _values.value(_ParmTypePeriod)->toInteger());
  if (! line)
  {
    Command::error("CommandRSI::runScript: getRSI");
    return;
  }
