...
int APO::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, QStringList &maList, BarData *data)
{
  // INDICATOR,APO,<NAME>,<INPUT>,<FAST_PERIOD>,<SLOW_PERIOD>,<MA_TYPE>

  if (set.count() != 7)
  {
    qDebug() << "APO::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "APO::calculate: invalid fast period parm" << set[4];
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "APO::calculate: invalid slow period parm" << set[5];
    return 1;
  }
  ...
  if (ma == -1)
  {
    qDebug() << "APO::calculate: invalid ma parm" << set[6];
    return 1;
  }
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << "APO::calculate: TA-Lib error" << rc;
    return 1;
  }
