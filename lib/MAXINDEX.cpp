...
int MAXINDEX::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
  ...
  PlotLine *line = getMAXINDEX(in, period);
  if (! line)
  {
    delete in;
    return 1;
...
int MAXINDEX::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
...
PlotLine * MAXINDEX::getMAXINDEX (PlotLine *in, int period)
{
  int size = in->getSize();
  TA_Real input[size];
  TA_Integer out[size];
  ...
  TA_Integer outBeg;
  TA_Integer outNb;
  TA_RetCode rc = TA_MAXINDEX(0, size - 1, &input[0], period, &outBeg, &outNb, &out[0]);
  if (rc != TA_SUCCESS)
  {
