...
int STDDEV::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,STDDEV,<NAME>,<INPUT>,<PERIOD>,<DEVIATION>

  if (set.count() != 6)
  {
    qDebug() << "STDDEV::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "STDDEV::calculate: invalid period parm" << set[4];
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "STDDEV::calculate: invalid dev" << set[5];
    return 1;
  }
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << "STDDEV::calculate: TA-Lib error" << rc;
    return 1;
  }
