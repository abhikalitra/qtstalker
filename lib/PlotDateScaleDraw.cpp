...
PlotDateScaleDraw::setDates ()
{
  ...
  DateRange dr;
  for (int pos = 0; pos < g_symbol->bars(); pos++)
  {
    Bar *b = g_symbol->bar(pos);
    QDateTime ted = dr.interval(b->date(), g_symbol->length());
...
PlotDateScaleDraw::label (double v) const
{
  ...
  switch ((BarLength::Key) g_symbol->length())
  {
...
PlotDateScaleDraw::draw (QPainter *painter, const QPalette &) const
{
  ...
  if ((BarLength::Key) g_symbol->length() != BarLength::_MINUTE1)
    ...
    switch ((BarLength::Key) g_symbol->length())
    {
      ...
      case BarLength::_MINUTE60:
      {
        ...
        else
        {
          if (dt >= nextHour)
          {
            if ((BarLength::Key) g_symbol->length() < BarLength::_MINUTE30)
            {
              // draw the short tick
...
PlotDateScaleDraw::dateToX (QDateTime d)
{
  int x = -1;
  DateRange dr;
  QDateTime ted = dr.interval(d, g_symbol->length());
