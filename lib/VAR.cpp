...
int VAR::calculate (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
  // INDICATOR,VAR,<NAME>,<INPUT>,<PERIOD>,<DEVIATION>

  if (set.count() != 6)
  {
    qDebug() << "VAR::calculate: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "VAR::calculate: invalid period parm" << set[4];
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "VAR::calculate: invalid dev" << set[5];
    return 1;
  }
