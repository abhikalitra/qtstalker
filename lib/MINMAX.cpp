...
int MINMAX::getIndicator (Indicator &ind, BarData *data)
{
  QString s;
...
int MINMAX::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
...
int MINMAX::getMINMAX (PlotLine *in, int period, QList<PlotLine *> &l)
{
  int size = in->getSize();
  TA_Integer outBeg;
  TA_Integer outNb;
  ...
  TA_RetCode rc = TA_MINMAX(0, size - 1, &input[0], period, &outBeg, &outNb, &out[0], &out2[0]);
  if (rc != TA_SUCCESS)
  {
