...
int DX::getIndicator (Indicator &ind, BarData *data)
{
  int period = settings.getInt(periodKey);

  PlotLine *line = getDX(data, period);
  if (! line)
    return 1;
...
int DX::getCUS (QStringList &set, QHash<QString, PlotLine *> &tlines, BarData *data)
{
...
PlotLine * DX::getDX (BarData *data, int period)
{
  int size = data->count();
  TA_Real high[size];
  TA_Real low[size];
