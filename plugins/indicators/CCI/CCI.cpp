...
int CCI::getIndicator (Indicator &ind, BarData &data)
{
  ...
  Curve *line = calculate(period, smoothing, type, data);
  if (! line)
    return 1;
...
int CCI::getCUS (QStringList &set, Indicator &ind, BarData &data)
{
  ...
  if (set.count() != 7)
  {
    qDebug() << "CCI::getCUS: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (tl)
  {
    qDebug() << "CCI::getCUS: duplicate name" << set[3];
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "CCI::getCUS: invalid period parm" << set[4];
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "CCI::getCUS: invalid smoothing period" << set[5];
    return 1;
  }
  ...
  if (ma == -1)
  {
    qDebug() << "CCI::getCUS: invalid smoothing type" << set[6];
    return 1;
  }
...
Curve * CCI::calculate (int period, int smoothing, int type, BarData &data)
{
  int size = data.count();
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << "CCI::calculate: TA-Lib error" << rc;
    return 0;
  }
