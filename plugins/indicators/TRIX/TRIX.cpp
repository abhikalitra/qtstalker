...
int TRIX::getCUS (QStringList &set, Indicator &ind, BarData &data)
{
  ...
  if (set.count() != 6)
  {
    qDebug() << "TRIX::getCUS: invalid parm count" << set.count();
    return 1;
  }
  ...
  if (tl)
  {
    qDebug() << "TRIX::getCUS: duplicate name" << set[3];
    return 1;
  }
  ...
  if (! in)
  {
    ...
    if (! in)
    {
      qDebug() << "TRIX::getCUS: input not found" << set[4];
      return 1;
    }
  ...
  if (! ok)
  {
    qDebug() << "TRIX::getCUS: invalid period parm" << set[5];
    return 1;
  }
...
Curve * TRIX::calculate (Curve *in, int period)
{
  if (in->count() < period)
    return 0;
