...
MOMDialog::MOMDialog (QWidget *p) : QDialog (p)
{
  _settings = 0;
  _modified = 0;
  _helpFile = "main.html";
...
void MOMDialog::cancel ()
{
  saveSettings();
  reject();
}

void MOMDialog::loadSettings ()
{
  QSettings settings(g_settings);
  settings.beginGroup(g_session);

  QSize sz = settings.value("mom_dialog_size", QSize(200,200)).toSize();
  resize(sz);

  // restore the position of the app
  QPoint p = settings.value("mom_dialog_pos").toPoint();
  if (! p.isNull())
    move(p);
}

void MOMDialog::saveSettings ()
{
  QSettings settings(g_settings);
  settings.beginGroup(g_session);
  settings.setValue("mom_dialog_size", size());
  settings.setValue("mom_dialog_pos", pos());
}

void MOMDialog::setGUI (Entity *settings)
{
  _settings = settings;
  ...
  // show
  set = _settings->get(QString("momShow"));

  _momShow = new QCheckBox;
  _momShow->setChecked(set->toBool());
  connect(_momShow, SIGNAL(toggled(bool)), this, SLOT(modified()));
  form->addRow(tr("Show"), _momShow);

  _tabs->addTab(w, QString("MOM"));
...
void MOMDialog::save ()
{
  ...
  set = _settings->get(QString("momShow"));
  set->setValue(_momShow->isChecked());
