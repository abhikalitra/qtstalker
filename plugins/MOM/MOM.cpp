...
int
MOM::dialog (PluginData *pd)
{
  if (! pd->dialogParent)
  {
    qDebug() << "MOM::dialog: invalid parent";
    return 0;
  }
  
  if (! pd->settings)
  {
    qDebug() << "MOM::dialog: invalid settings";
    return 0;
  }
  
  MOMDialog *dialog = new MOMDialog(pd->dialogParent);
...
MOM::run (PluginData *pd)
{
  ...
  if (! getMOM(input->toString(), period->toInt(), label->toString()))
    return 0;
  
  // mom
  QVariant *show = pd->settings->get(QString("momShow"));
  if (! show)
    return 0;
...
int
MOM::getMOM (QString ikey, int period, QString okey)
{
  if (! g_symbol)
    return 0;
  ...
  rc = TA_MOM (0, dpos - 1, &input[0], period, &outBeg, &outNb, &out[0]);
  
  if (rc != TA_SUCCESS)
  {
    qDebug() << "MOM::getMOM: TA-Lib error" << rc;
    return 0;
  }
...
int
MOM::getMA (QString inKey, QString outKey, int type, int period)
{
  if (! g_symbol)
    return 0;
...
MOM::settings (PluginData *pd)
{
  ...
  command->set(QString("plugin"), new QVariant(QString("MOM")));
  ...
  command->set(QString("label"), new QVariant(QString("MOM")));
  command->set(QString("momShow"), new QVariant(TRUE));
...
int
MOM::draw (QPainter *, const QwtScaleMap &, const QwtScaleMap &, const QRect &, void *)
{
  return 0;
}
