...
DialogUser *
SCRIPT_PLOT_LINE::dialog (QWidget *p, Entity *settings)
{
...
int
SCRIPT_PLOT_LINE::run (Entity *settings, QList<Curve *> &output, QList<Marker *> &)
{
  if (! g_symbol)
    return 0;
  ...
  line->fill(input->toString(), QString(), QString(), QString(), color->toColor());
  line->setZ(0);
  output << line;
  
//qDebug() << "SCRIPT_PLOT_LINE::run:" << input->toString() << label->toString();

  return 1;
}

Entity *
SCRIPT_PLOT_LINE::settings ()
{
  ...
  set->setString(QString("SCRIPT_PLOT_LINE"), QString(), -1);
  ...
  CurveLineType style;
  set = new Setting;
  set->setCombo(style.list(), style.indexToString(CurveLineType::_SOLID), tr("Style"), order++);
...
// do not remove
Q_EXPORT_PLUGIN2(script_plot_line, SCRIPT_PLOT_LINE);
