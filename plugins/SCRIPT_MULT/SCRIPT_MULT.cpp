...
#include "SCRIPT_MULT.h"
#include "BarType.h"
#include "Global.h"


DialogUser *
SCRIPT_MULT::dialog (QWidget *p, Entity *settings)
{
...
int
SCRIPT_MULT::run (Entity *settings, QList<Curve *> &, QList<Marker *> &)
{
  if (! g_symbol)
    return 0;
  ...
  if (! getMULT(input->toString(), input2->toString(), label->toString()))
    return 0;
...
int
SCRIPT_MULT::getMULT (QString ikey, QString i2key, QString okey)
{
  if (! g_symbol)
    return 0;
  ...
  if (rc != TA_SUCCESS)
    qDebug() << "SCRIPT_MULT::getMULT: error on TA_Initialize";
  ...
  rc = TA_MULT (0, dpos - 1, &input[0], &input2[0], &outBeg, &outNb, &out[0]);
  
  if (rc != TA_SUCCESS)
  {
    qDebug() << "SCRIPT_MULT::getMULT: TA-Lib error" << rc;
    return 0;
  }
...
SCRIPT_MULT::settings ()
{
  ...
  set->setString(QString("SCRIPT_MULT"), QString(), -1);
  ...
  // label
  set = new Setting;
  set->setString(QString("MULT"), tr("Label"), order++);
...
// do not remove
Q_EXPORT_PLUGIN2(script_mult, SCRIPT_MULT);
