...
qtsFFT::qtsFFT(long n) : _ftt_Real(n)
{
    fftReal = new double[n];	// ftt arrays
    fftFreq = new double[n];
...
PlotLine * qtsFFT::do_FFTqts(PlotLine *r)
{
    ...
    for (i = 0; i < length; i++)
    {
        fftReal[i] = r->getData(i); 
    }
    
    _ftt_Real.do_fft(fftFreq, fftReal);

    for (i = 0; i < length; i++)
    {
        result->append(fftFreq[i]);
    }
...
qtsFFT::FFTReal::FFTReal (const long length)
:	_length (length)
...
void	qtsFFT::FFTReal::do_fft (flt_t f [], const flt_t x []) const
{
...
void	qtsFFT::FFTReal::do_ifft (const flt_t f [], flt_t x []) const
{
...
qtsFFT::FFTReal::BitReversedLUT::BitReversedLUT (const int nbr_bits)
{
	long				length;
	long				cnt;
	long				br_index;
...
qtsFFT::FFTReal::BitReversedLUT::~BitReversedLUT (void)
{
	delete [] _ptr;
	_ptr = 0;
}
...
qtsFFT::FFTReal::TrigoLUT::TrigoLUT (const int nbr_bits)
{
	long		total_len;
