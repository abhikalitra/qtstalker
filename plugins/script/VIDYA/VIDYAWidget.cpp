...
VIDYAWidget::VIDYAWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void VIDYAWidget::createGeneralPage ()
{
  ...
  _vperiod->setRange(1, 9999);
  form->addRow("PERIOD_VOLUME", _vperiod);
}

void VIDYAWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "VIDYA")
    return;

  _output->setText(command.parm("NAME"));
  _input->setText(command.parm("INPUT"));
  _period->setValue(command.parm("PERIOD").toInt());
  _vperiod->setValue(command.parm("PERIOD_VOLUME").toInt());
}

void VIDYAWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=VIDYA";
  ...
  cl << "PERIOD_VOLUME=" + _vperiod->text();
