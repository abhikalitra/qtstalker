...
MOMWidget::MOMWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void MOMWidget::createGeneralPage ()
{
...
void MOMWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "MOM")
    return;
...
void MOMWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=MOM";
