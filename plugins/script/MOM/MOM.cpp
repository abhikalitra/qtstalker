...
int MOM::calculate (BarData *bd, Indicator *i, Setting *settings)
{
...
Curve * MOM::getMOM (QList<Curve *> &list, int period)
{
  if (! list.count())
    return 0;
...
QWidget * MOM::dialog (QWidget *p, Setting *set)
{
  return new MOMDialog(p, set);
}
