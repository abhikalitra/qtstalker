...
CCIWidget::CCIWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void CCIWidget::createGeneralPage ()
{
...
void CCIWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "CCI")
    return;
...
void CCIWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=CCI";
