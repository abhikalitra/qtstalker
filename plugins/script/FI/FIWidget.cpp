...
FIWidget::FIWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void FIWidget::createGeneralPage ()
{
...
void FIWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "FI")
    return;
...
void FIWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=FI";
