...
ADXWidget::ADXWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void ADXWidget::createGeneralPage ()
{
...
void ADXWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "ADX")
    return;
...
void ADXWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=ADX";
