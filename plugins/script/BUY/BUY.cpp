...
int BUY::request (Setting *request, Setting *data)
{
  QStringList l;
...
int BUY::addItem (Setting *data)
{
  BuyDraw *co = new BuyDraw;
...
int BUY::info (Setting *info)
{
  BuyDraw *item = _items.value(info->data("ID"));
  if (! item)
    return 1;
...
int BUY::highLow (Setting *request, Setting *data)
{
  double high = 0;
  double low = 0;
  int flag = 0;
  ...
  QHashIterator<QString, BuyDraw *> it(_items);
  while (it.hasNext())
  {
    it.next();
...
void BUY::click (int button, QPoint p)
{
...
int BUY::create (Setting *parms)
{
  _selected = new BuyDraw;
  Setting *set = _selected->settings();
...
void BUY::dialog ()
{
  ...
  QWidget *w = new BuyDialog(_dialog, _selected->settings());
...
int BUY::calculate (BarData *, Indicator *i, Setting *settings)
{
...
int BUY::command (Command *command)
{
  // PARMS:
  // TYPE
  // NAME
...
QWidget * BUY::dialog (QWidget *p, Setting *set)
{
  return new BuyDialog(p, set);
}

void BUY::defaults (Setting *set)
{
