...
ARITHMETICWidget::ARITHMETICWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
  loadSettings();
}

void ARITHMETICWidget::createGeneralPage ()
{
  // toolbar
...
void ARITHMETICWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "ARITHMETIC")
    return;
...
void ARITHMETICWidget::selectionChanged ()
{
  bool status = TRUE;
...
void ARITHMETICWidget::addItem ()
{
  addItem(QString("add"), QString("ADD"), QString("Close"), QString("Close"));
}

void ARITHMETICWidget::addItem (QString name, QString method, QString input, QString input2)
{
...
void ARITHMETICWidget::deleteItem ()
{
  QList<QTreeWidgetItem *> l = _tree->selectedItems();
  if (! l.count())
    return;
...
void ARITHMETICWidget::loadSettings ()
{
  ...
  {
    QString key = "ARITHMETICWidget_rule_column_width_" + QString::number(loop);
    _tree->setColumnWidth(loop, set.value(key, 75).toInt());
  }
}

void ARITHMETICWidget::commands (QStringList &l, int tab)
{
  int loop = 0;
