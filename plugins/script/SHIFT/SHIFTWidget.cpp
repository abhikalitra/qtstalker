...
SHIFTWidget::SHIFTWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
  loadSettings();
}

void SHIFTWidget::createGeneralPage ()
{
  // toolbar
...
void SHIFTWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "SHIFT")
    return;
...
void SHIFTWidget::selectionChanged ()
{
  bool status = TRUE;
...
void SHIFTWidget::addItem ()
{
  addItem(QString("shift"), QString("Close"), QString("1"));
}

void SHIFTWidget::addItem (QString name, QString input, QString period)
{
...
void SHIFTWidget::loadSettings ()
{
  QSettings set(g_globalSettings);
  int loop = 0;
  for (; loop < _tree->columnCount(); loop++)
  {
    QString key = "SHIFTWidget_rule_column_width_" + QString::number(loop);
    _tree->setColumnWidth(loop, set.value(key, 75).toInt());
  }
}

void SHIFTWidget::commands (QStringList &l, int tab)
{
  int loop = 0;
