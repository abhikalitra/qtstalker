...
TRIXWidget::TRIXWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void TRIXWidget::createGeneralPage ()
{
...
void TRIXWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "TRIX")
    return;
...
void TRIXWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=TRIX";
