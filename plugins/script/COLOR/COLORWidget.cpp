...
COLORWidget::COLORWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
  loadSettings();
}

void COLORWidget::createGeneralPage ()
{
  // toolbar
...
void COLORWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "COLOR")
    return;
...
void COLORWidget::selectionChanged ()
{
  bool status = TRUE;
...
{
  addItem(QString("#FF0000"), QString("Close"), QString("EQ"), QString("Close"), QString("Close"));
}

void COLORWidget::addItem (QString color, QString name, QString op, QString name2, QString name3)
{
...
void COLORWidget::loadSettings ()
{
  QSettings set(g_globalSettings);
  int loop = 0;
  for (; loop < _tree->columnCount(); loop++)
  {
    QString key = "COLORWidget_rule_column_width_" + QString::number(loop);
    _tree->setColumnWidth(loop, set.value(key, 75).toInt());
  }
}

void COLORWidget::commands (QStringList &l, int tab)
{
  int loop = 0;
