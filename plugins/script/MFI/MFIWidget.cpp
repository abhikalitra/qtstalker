...
MFIWidget::MFIWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void MFIWidget::createGeneralPage ()
{
...
void MFIWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "MFI")
    return;
...
void MFIWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=MFI";
