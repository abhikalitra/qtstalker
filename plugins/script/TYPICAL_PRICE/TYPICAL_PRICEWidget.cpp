...
TYPICAL_PRICEWidget::TYPICAL_PRICEWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
  loadSettings();
}

void TYPICAL_PRICEWidget::createGeneralPage ()
{
  // toolbar
...
void TYPICAL_PRICEWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "TYPICAL_PRICE")
    return;
...
void TYPICAL_PRICEWidget::selectionChanged ()
{
  bool status = TRUE;
...
void TYPICAL_PRICEWidget::addItem ()
{
  addItem(QString("AP"), QString("High"), QString("Low"), QString("Close"));
}

void TYPICAL_PRICEWidget::addItem (QString name, QString high, QString low, QString close)
{
...
void TYPICAL_PRICEWidget::deleteItem ()
{
  QList<QTreeWidgetItem *> l = _tree->selectedItems();
  if (! l.count())
    return;
...
void TYPICAL_PRICEWidget::loadSettings ()
{
  QSettings set(g_globalSettings);
  int loop = 0;
  for (; loop < _tree->columnCount(); loop++)
  {
    QString key = "TYPICAL_PRICEWidget_rule_column_width_" + QString::number(loop);
    _tree->setColumnWidth(loop, set.value(key, 75).toInt());
  }
}

void TYPICAL_PRICEWidget::commands (QStringList &l, int tab)
{
  int loop = 0;
