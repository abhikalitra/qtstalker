...
ROCWidget::ROCWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void ROCWidget::createGeneralPage ()
{
...
void ROCWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "ROC")
    return;
...
void ROCWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=ROC";
