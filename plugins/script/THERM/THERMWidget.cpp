...
THERMWidget::THERMWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void THERMWidget::createGeneralPage ()
{
...
void THERMWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "THERM")
    return;
...
void THERMWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=THERM";
