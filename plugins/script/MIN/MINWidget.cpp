...
MINWidget::MINWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
  loadSettings();
}

void MINWidget::createGeneralPage ()
{
  // toolbar
...
void MINWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "MIN")
    return;
...
void MINWidget::selectionChanged ()
{
  bool status = TRUE;
...
void MINWidget::addItem ()
{
  addItem(QString("MIN"), QString("Close"), QString("10"));
}

void MINWidget::addItem (QString name, QString input, QString period)
{
...
void MINWidget::loadSettings ()
{
  QSettings set(g_globalSettings);
  int loop = 0;
  for (; loop < _tree->columnCount(); loop++)
  {
    QString key = "MINWidget_rule_column_width_" + QString::number(loop);
    _tree->setColumnWidth(loop, set.value(key, 75).toInt());
  }
}

void MINWidget::commands (QStringList &l, int tab)
{
  int loop = 0;
