...
BETAWidget::BETAWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void BETAWidget::createGeneralPage ()
{
...
void BETAWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "BETA")
    return;
...
void BETAWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=BETA";
