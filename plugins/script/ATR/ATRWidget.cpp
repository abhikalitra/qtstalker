...
ATRWidget::ATRWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void ATRWidget::createGeneralPage ()
{
...
void ATRWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "ATR")
    return;
...
void ATRWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=ATR";
