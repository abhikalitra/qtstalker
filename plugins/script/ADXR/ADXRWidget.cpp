...
ADXRWidget::ADXRWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void ADXRWidget::createGeneralPage ()
{
...
void ADXRWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "ADXR")
    return;
...
void ADXRWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=ADXR";
