...
Curve * WILLR::getWILLR (QList<Curve *> &list, int period)
{
  if (list.count() != 3)
    return 0;
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << _plugin << "::getWILLR: TA-Lib error" << rc;
    return 0;
  }
...
PluginWidget * WILLR::dialog (QWidget *p)
{
  return new WILLRWidget(p);
}

void WILLR::defaults (QString &d)
{
  QStringList l;
  l << "PLUGIN=" + _plugin;
  l << "NAME=" + _plugin;
