...
WILLRWidget::WILLRWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void WILLRWidget::createGeneralPage ()
{
...
void WILLRWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "WILLR")
    return;
...
void WILLRWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=WILLR";
