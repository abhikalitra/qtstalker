...
{
  // CHART_OBJECT_SELL,<NAME>,<INDICATOR>,<EXCHANGE>,<SYMBOL>,<DATE>,<PRICE>,<COLOR>
  //         0           1         2         3         4       5       6       7
  ...
  if (! i)
  {
    qDebug() << "CHART_OBJECT_SELL::command: no indicator";
    return 1;
  }
  ...
  if (! dt.isValid())
  {
    qDebug() << "CHART_OBJECT_SELL::command: invalid date" << command->parm(pos);
    return 1;
  }
  ...
  if (! ok)
  {
    qDebug() << "CHART_OBJECT_SELL::command: invalid price" << command->parm(pos);
    return 1;
  }
  ...
  if (! color.isValid())
  {
    qDebug() << "CHART_OBJECT_SELL::command: invalid color" << command->parm(pos);
    return 1;
  }
