...
STOCH_RSIWidget::STOCH_RSIWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void STOCH_RSIWidget::createGeneralPage ()
{
...
void STOCH_RSIWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "STOCH_RSI")
    return;
...
void STOCH_RSIWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=STOCH_RSI";
