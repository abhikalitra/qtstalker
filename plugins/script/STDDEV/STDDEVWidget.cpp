...
STDDEVWidget::STDDEVWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void STDDEVWidget::createGeneralPage ()
{
...
void STDDEVWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "STDDEV")
    return;
...
void STDDEVWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=STDDEV";
