...
Curve * STDDEV::getSTDDEV (QList<Curve *> &list, int period, double dev)
{
  if (! list.count())
    return 0;
  ...
  if (rc != TA_SUCCESS)
  {
    qDebug() << _plugin << "::getSTDDEV: TA-Lib error" << rc;
    return 0;
  }
...
PluginWidget * STDDEV::dialog (QWidget *p)
{
  return new STDDEVWidget(p);
}

void STDDEV::defaults (QString &d)
{
  QStringList l;
  l << "PLUGIN=" + _plugin;
  l << "NAME=" + _plugin;
