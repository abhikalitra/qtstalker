...
CORRELWidget::CORRELWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void CORRELWidget::createGeneralPage ()
{
...
void CORRELWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "CORREL")
    return;

  _output->setText(command.parm("NAME"));
  _input->setText(command.parm("INPUT"));
  _index->setText(command.parm("INDEX"));
  _period->setValue(command.parm("PERIOD").toInt());
}

void CORRELWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=CORREL";
  cl << "NAME=" + _output->text();
  cl << "INPUT=" + _input->text();
  cl << "INDEX=" + _index->text();
