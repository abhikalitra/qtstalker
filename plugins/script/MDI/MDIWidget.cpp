...
MDIWidget::MDIWidget (QWidget *p) : PluginWidget (p)
{
  createGeneralPage();
}

void MDIWidget::createGeneralPage ()
{
...
void MDIWidget::setCommand (QString d)
{
  Command command(d);
  if (command.parm("PLUGIN") != "MDI")
    return;
...
void MDIWidget::commands (QStringList &l, int tab)
{
  QStringList cl;
  cl << "PLUGIN=MDI";
