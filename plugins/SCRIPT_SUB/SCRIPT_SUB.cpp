...
#include "SCRIPT_SUB.h"
#include "BarType.h"
#include "Global.h"


DialogUser *
SCRIPT_SUB::dialog (QWidget *p, Entity *settings)
{
...
int
SCRIPT_SUB::run (Entity *settings, QList<Curve *> &, QList<Marker *> &)
{
  if (! g_symbol)
    return 0;
  ...
  if (! getSUB(input->toString(), input2->toString(), label->toString()))
    return 0;
...
int
SCRIPT_SUB::getSUB (QString ikey, QString i2key, QString okey)
{
  if (! g_symbol)
    return 0;
  ...
  if (rc != TA_SUCCESS)
    qDebug() << "SCRIPT_SUB::getSUB: error on TA_Initialize";
  ...
  rc = TA_SUB (0, dpos - 1, &input[0], &input2[0], &outBeg, &outNb, &out[0]);
  
  if (rc != TA_SUCCESS)
  {
    qDebug() << "SCRIPT_SUB::getSUB: TA-Lib error" << rc;
    return 0;
  }
...
SCRIPT_SUB::settings ()
{
  ...
  set->setString(QString("SCRIPT_SUB"), QString(), -1);
  ...
  // label
  set = new Setting;
  set->setString(QString("SUB"), tr("Label"), order++);
...
// do not remove
Q_EXPORT_PLUGIN2(script_sub, SCRIPT_SUB);
