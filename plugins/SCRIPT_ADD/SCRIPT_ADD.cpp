...
#include "SCRIPT_ADD.h"
#include "BarType.h"
#include "Global.h"


DialogUser *
SCRIPT_ADD::dialog (QWidget *p, Entity *settings)
{
...
int
SCRIPT_ADD::run (Entity *settings, QList<Curve *> &, QList<Marker *> &)
{
  if (! g_symbol)
    return 0;
  ...
  if (! getADD(input->toString(), input2->toString(), label->toString()))
    return 0;
...
int
SCRIPT_ADD::getADD (QString ikey, QString i2key, QString okey)
{
  if (! g_symbol)
    return 0;
  ...
  if (rc != TA_SUCCESS)
    qDebug() << "SCRIPT_ADD::getADD: error on TA_Initialize";
  ...
  rc = TA_ADD (0, dpos - 1, &input[0], &input2[0], &outBeg, &outNb, &out[0]);
  
  if (rc != TA_SUCCESS)
  {
    qDebug() << "SCRIPT_ADD::getADD: TA-Lib error" << rc;
    return 0;
  }
...
SCRIPT_ADD::settings ()
{
  ...
  set->setString(QString("SCRIPT_ADD"), QString(), -1);
  ...
  // label
  set = new Setting;
  set->setString(QString("ADD"), tr("Label"), order++);
...
// do not remove
Q_EXPORT_PLUGIN2(script_add, SCRIPT_ADD);
